...
docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "cxc-demo/grafana:${_grafana_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "cxc-demo/grafana:latest"  --no-cache=true .
fi
